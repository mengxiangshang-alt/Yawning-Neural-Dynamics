...
"""
Neural Dynamics Trajectory, Velocity, and Morphological Statistics.
This script generates the statistical subplots (e.g., Figure 5 D, H, L, P) 
to evaluate macroscopic network suppression and recovery trajectories in Beagle dog EEG.
It incorporates dynamic time warping (DTW) for sequence alignment, Gaussian-smoothed 
derivatives for dynamic velocity, and Gardner-Altman estimation plots for robust statistics.
"""

# ==========================================
# 0. Global Configuration & Paths 
# ==========================================
# Please modify these paths to match your local directory structure
...
# ==========================================
# Top-level Plotting Style Settings
...
def calculate_metrics_trajectory(raw, window_sec, step_sec):
    ...
    for i in range(n_steps):
        ...
        # B. Gamma Energy (Hilbert Envelope Mean)
...
def process_group_data(json_path):
    """Read raw files, preprocess, align, and normalize features."""
    ...
    for fpath, times in target_dict.items():
        ...
        try:
            ...
            tmin, tmax = max(0, t0 - CONFIG['T_WINDOW'] - 2), min(raw.times[-1], t0 + 2)
            if tmax <= tmin: 
                continue
            raw.crop(tmin, tmax)
...
def calculate_slope(data_list, time_axis, window=(-30, -5)):
    """Calculate the linear trend slope within a specific time window."""
...
def get_smooth_derivative(data_matrix, sigma=2.0):
    """
    Calculate dynamic velocity via smoothed first derivative.
    Applies Gaussian smoothing followed by gradient computation.
    """
...
def draw_estimation_plot(ax_main, data_ctrl, data_yawn, ylabel, title):
    ...
    Strictly forces statistical lines to 0.5pt.
    """
    positions = [0, 1]
    ...
    if len(data_ctrl) > 2 and len(data_yawn) > 2:
        ...
        # Terminal Output for statistical records
...
def draw_dtw_visual(ax, time_axis, trace_yawn, trace_ctrl_mean, title):
    """Draw DTW Alignment Path Plot to highlight temporal warping."""
    if not HAS_DTW: 
        return
...
# ==========================================
# 4. Main Program: Generate Statistics Array
# ==========================================
def run_figure5_statistics():
    print("Step 1: Loading and Processing Temporal Data...")
    yawn_res = process_group_data(PATH_YAWN) 
    ctrl_res = process_group_data(PATH_CONTROL) 
    
    if yawn_res is None or ctrl_res is None:
        print("Error: Could not load target data. Please check path configurations.")
        return
    ...
    print("Step 2: Generating Figure 5 Statistics Panel (8 Subplots)...")
    
    fig = plt.figure(figsize=(16, 9))
    gs = fig.add_gridspec(2, 4, width_ratios=[1.2, 1.2, 1.0, 0.8], wspace=0.35, hspace=0.4)
    
    # ================= ROW 1: Complexity (LZC) =================
    
    # Trajectory
    ax1 = fig.add_subplot(gs[0, 0])
    draw_trajectory(ax1, t, y_lzc, c_lzc, 'Complexity Dynamics (LZC)', 'LZC (Z)')
    
    # Velocity
    ax2 = fig.add_subplot(gs[0, 1])
    draw_trajectory(ax2, t, y_lzc_vel, c_lzc_vel, 'LZC Rate of Change', 'd(LZC)/dt')
    ...
    if len(y_lzc) > 0 and len(c_lzc) > 0:
        ...
        draw_dtw_visual(ax3, t, y_lzc[idx], tpl_l, 'LZC Pattern Shift\n(DTW Visual)')
        
    # Slope Stat (Figure 5 Subplot D)
    ax4 = fig.add_subplot(gs[0, 3])
    draw_estimation_plot(ax4, c_lzc_slope, y_lzc_slope, 'Slope (k)', 'Trend Analysis\n(Complexity)')

    # ================= ROW 2: Effort (Gamma) =================
    
    # Trajectory
    ax5 = fig.add_subplot(gs[1, 0])
    draw_trajectory(ax5, t, y_gam, c_gam, 'Gamma Energy Effort', 'Gamma Amp (Z)')
    
    # Velocity
    ax6 = fig.add_subplot(gs[1, 1])
    draw_trajectory(ax6, t, y_gam_vel, c_gam_vel, 'Gamma Rate of Change', 'd(Gamma)/dt')
    ...
    if len(y_gam) > 0 and len(c_gam) > 0:
        ...
        draw_dtw_visual(ax7, t, y_gam[idx_g], tpl_g, 'Gamma Pattern Shift\n(DTW Visual)')
        
    # Slope Stat (Figure 5 Subplot H)
    ax8 = fig.add_subplot(gs[1, 3])
    draw_estimation_plot(ax8, c_gam_slope, y_gam_slope, 'Slope (k)', 'Effort Trend\n(Struggle Strength)')

    fig.suptitle("Figure 5: Neural Dynamics of State Transition (Trajectory, Velocity, Morphology & Statistics)", 
                 fontsize=14, fontweight='bold', y=0.98)
    
    save_path = os.path.join(SAVE_DIR, "Figure_5_DHLP_Trajectory_Statistics.pdf")
    fig.savefig(save_path, bbox_inches='tight', dpi=300)
    
    print(f"\nExecution Complete! Statistical chart saved to: {save_path}")
    plt.show()

if __name__ == "__main__":
    run_figure5_statistics()